...
> 状态：既有 V1—V1.4 页面行为、云南省开奖公告、期号入口与奖情内联已完成实现和本轮验证；源码与 debug 测试 APK 已进入 GitHub Draft Release；正式发行门禁仍未关闭
> 日期：2026-08-12
> 输入：`PRD-lucky3D.md` V1.4、云南省奖情与期号入口规格、已批准原型、开发前技术设计、实时内容设计、数据同步设计、计算规则V1
...
V1.4 把首页试机号保留到次日北京时间 `16:30`，期号移入走势图统一横向内容首列，并压缩彩报标题与换期操作；详细行为以 `docs/superpowers/specs/2026-08-09-lucky3d-v1.4-display-and-navigation.md` 为准。
...
- [x] Android 映射更新为 `versionName 1.3.0` / `versionCode 3`；未单独发布，已并入 V1.4.0。

## 15. V1.4 展示时序与页面导航

本节由 `docs/superpowers/specs/2026-08-09-lucky3d-v1.4-display-and-navigation.md` 批准，只替代 V1.2/V1.3 的试机号显示时序、固定期号栏和彩报标题/换期布局；历史验收记录和未被替代的数据、缓存、计算与发布门禁保持不变。

版本映射：产品里程碑 `V1.4` 对应 Android `versionName 1.4.0` / `versionCode 4`。

1. **Task 40：规格与文档对齐** — 固化三个已确认行为，同步 PRD、数据同步、设计、版本和任务文档，并对旧规格增加替代说明。
   - 状态：已完成文档对齐；不以文档更新代替实现与验证。
2. **Task 41：首页 16:30 展示边界** — 昨日试机号跨日保留到 `16:30`；到点后今日记录未写入时显示 `---`，成功刷新后显示新号；正式开奖同步不再使保留窗口内试机号提前失效。
   - 验收：覆盖 `16:29:59`、`16:30:00`、前台跨界、官方期号追平、自动失败和手动失败。
   - 状态：已完成，时间边界与失败状态单元测试通过，API 26/API 37 首页设备测试通过。
3. **Task 42：走势图统一期号列** — 删除固定期号绘制区，把期号作为统一横向内容首列，并让表头、表体、未来行和统计行共享同一横向偏移、缩放与命中坐标。
   - 验收：从期号列开始横拖时整表同步移动；全览、放大、横纵拖动和边界夹紧无错位或大面积空白。
   - 状态：已完成，统一 viewport 回归测试、API 26/API 37 设备测试和 API 26 实际横滑截图验收通过。
4. **Task 43：彩报标题与相邻换期** — `A11第三版` 以小字浅色与“彩报”同排；当前期号后紧邻上一期、下一期，并在最早/最新有效期号下分别禁用对应按钮。
   - 验收：上一期指向更早期号、下一期指向更晚期号；边界按钮同时具备禁用语义和视觉，触控目标不小于 `48dp`。
   - 状态：已完成，方向和双边界测试、API 26/API 37 设备测试及 API 26 实际换期截图验收通过。
5. **Task 44：版本与交付验证** — 更新 Android 版本，运行数据、JVM、lint、Debug/Release 构建和 API 26/API 37 相关设备验收，生成并核验新 APK。
   - 验收：只有本轮实际执行成功后，才在任务清单和发布记录中勾选对应门禁或写入 APK 身份。
   - 状态：已完成本地测试 APK、GitHub `main` 推送与 `v1.4.0` Draft Release；详细证据见 `docs/releases/v1.4.0.md`，正式发行门禁仍未关闭。

## 16. 2026-08-12 云南省奖情与期号入口实施

本节由 `docs/specs/2026-08-12-home-official-announcement-and-date-query.md`、`docs/proposals/2026-08-12-home-and-draw-query-redesign.md` 和 `docs/prototypes/2026-08-12-yunnan-announcement/` 的已批准原型共同约束。它替代此前“日期优先 + 公告摘要”的未实施方案；开奖号码同步契约保持不变，公告数据单独校验和存储。

实现状态由主代理在代码、单元测试、Compose 测试和目标模拟器验收后更新；本节只先写契约和门禁，不预先宣告完成。

1. **Task 45：期号唯一入口** — 首页仅保留当前期号可点击入口；日期作为不可点击元数据。查询页通过期号选择器按有效期号倒序切换，旧日期/年份/最近期数/范围模型仅作兼容。
   - 验收：首页只有期号语义可点击；日期无点击、筛选或导航语义；期号选择器触控目标至少 `48dp`，切换后结果按本地 Room Flow 更新。
2. **Task 46：云南省开奖公告数据契约** — 为独立云南省奖情定义按期号关联的公告记录：销售额、中奖总额、奖池资金余额（云南接口 `prizeLog.nextAmount`，领域/Room 为分 `Long`）、公告期号、开奖日期、独立的兑奖截止日（来源提供时）、三玩法中奖注数和单注奖金，以及可选动态派奖。
   - 验收：期号/日期/金额/玩法校验失败整条拒绝；奖池输入非负且最多两位小数，列表与详情缺失、非法或不一致不得入库，官方 `0L` 与旧库 `null` 可区分；三玩法按单选→组三→组六稳定排序；开奖结果同步不得填充或覆盖公告字段；公告不得跨期拼接到最新开奖号；缺失、离线和修正状态可恢复。
3. **Task 47：首页公告摘要与派奖显隐** — 首页标题固定“云南省开奖公告”，金额字段固定“销售额”；保留三玩法中奖注数/单注奖金，删除“基础中奖”可见分组标题。仅当存在正数派奖时显示一整行 `【派奖】`，按单选→组三→组六列出正数玩法及注数；无派奖时整行不存在。
   - 验收：有派奖/无派奖两种原型状态分别可见；正数玩法排序稳定，零值不进入派奖行；不得出现其他地区或全国奖情摘要。
4. **Task 48：查询页内联完整云南奖情** — 选定期号后按“开奖结果 → 云南省开奖公告 → 玩法奖情/派奖（如有） → 数据分析”连续展示；详情金额摘要同列销售额、中奖总额和奖池资金余额，兑奖截止日独立；公告元数据和三玩法字段完整，派奖明细只保留正数玩法；首页不显示奖池资金余额。
   - 验收：不使用底部弹层承载主要详情；只将公告与所选期号开奖结果配对，跨期旧公告不得拼接；无开奖记录或奖情缺失时状态明确且期号入口可修改；页面文案只使用批准的标题和金额标签，不添加来源/地域修饰词或“基础中奖”标题。
5. **Task 49：实施与验证门禁** — 原型已批准但实现未自动视为完成；由主代理执行相关 unit test、androidTest 编译/运行、目标模拟器视觉对比和全局 `rg` 文案审计后，再在 `tasks/todo.md` 勾选实际通过项。
   - 验收：验证命令和结果写入交付记录；任何未执行或失败的门禁保持未勾选。

## 17. 2026-08-13 奖池余额契约专项复核

1. **Task 53：奖池余额契约专项复核** — 实机反馈补充 `prizeLog.nextAmount` 的分单位、零值/旧库缺失和列表/详情一致性边界；仅在专项回归、Room 迁移和详情页 UI 证据齐全后更新任务状态。
   - 验收：覆盖非负金额与最多两位小数、`0L` 与 `null` 区分、列表/详情 mismatch/缺失/非法整条拒绝、详情展示而首页不展示、兑奖截止日独立及跨期公告隔离；无本轮证据时保持未完成。
2. **Task 54：公告与开奖号对齐及同步编排复核** — 公告提交前在 Room 事务内按期号、日期和三位开奖号校验本地 `draws`，读取时使用相同条件联表隔离；自动和手动刷新统一串行为“开奖号→云南公告”。
   - 验收：错日期、错号码、缺本地开奖或混合错误批次均不得部分写入；开奖号修正后旧公告不可读；自动/手动并发不得让公告先于在途开奖号同步，前导零号码保持正确。
