...
2. 彩经网历史页最多读取五个分页，仅补齐走势图可见窗口缺失期号；试机号不进入 `draws` 和任何计算。
3. 走势图把表格列模型、固定栏和统一 viewport 变换分开：固定栏只绘制期号，右侧共享缩放和横向偏移。
...
1. **Task 28：试机号分页解析** — 解析多行历史记录、分页来源和重复/非法页失败模型。
   - 验收：`007` 保留前导零；重复期号、错误表头、超限载荷整页拒绝。
   - 验证：`CjcpTrialHtmlParserTest`、`CjcpTrialDataSourceTest`。
2. **Task 29：试机号按期缓存** — Room DAO、Repository 按期查询与批量写入，保留旧期历史。
   - 验收：批量写入不删除已有期号；页面失败不污染缓存；100期最多请求五页。
...
5. **Task 32：走势图 viewport 缩放** — 动态 fitScale、双指缩放、全览/100%操作、命中坐标反算。
   - 验收：窄屏全览显示所有列；放大横滑和点选不乱位。
...
7. **Task 34：30天清理策略** — 30个北京时间自然日边界、按期查询和私有文件同步清理。
   ...
   - 验证：缓存边界、失败补偿和清理集成测试。
8. **Task 35：彩报期号选择与图片状态** — 上一期/当前期号/下一期、30天候选列表、切换重置缩放。
   - 验收：缓存命中不联网，未缓存只抓取所选期号；图片缩放/拖动/复位可用。
   - 验证：CaibaoViewModel、CaibaoScreen API 26/API 37 设备测试。

### Checkpoint：V1.2 数据与交互

- [ ] `test`、`lint`、`assembleDebug` 和种子数据校验通过。
- [ ] 走势图 10/30/60/100 期、全列缩放、固定期号栏和完整列顺序通过设备验收。
- [ ] 彩报30天缓存边界、最近30个自然日期号切换、离线读取和失败恢复通过设备验收。
- [ ] 设计 QA 覆盖窄屏、深浅色、48dp触控、TalkBack语义和真实数据密度。
