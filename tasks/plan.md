...
> 状态：Tasks 0—10 已完成并验证
> 日期：2026-07-28
> 输入：`PRD-lucky3D.md` V0.3、开发前技术设计、数据同步设计、计算规则V1

## 1. 概述

构建一个不依赖自建后端的原生Android应用。安装包内置2017001期起的福彩3D开奖，应用进入前台或手动刷新时直连福利彩票官方数据源增量更新。用户可以查看开奖和走势、组合条件筛选号码、保存方案与模板，并对历史和新开奖进行回测与复盘。

本计划包含静态只读彩报占位页，但不包含彩报联网更新、真实彩报聚合、云服务、账号系统、后台准点推送、在线购彩、支付、社区和AI荐号。
...
只有一个人工准入门槛：产品负责人批准PRD、技术设计和本实施计划。应用名、包名、最低系统和数据起点已有默认决策，不再阻塞开工。
...
- [ ] 五个入口可切换并保留各自主要页面状态。
- [ ] 彩报页离线可见，且没有网络更新、翻页、收藏、分享或购买入口。
- [ ] 深浅色模式和系统字体放大下无关键内容截断。
- [ ] 同步错误组件允许重试且不遮挡已有数据。
...
- [ ] 无网时立即显示本地最新一期和上次更新时间。
- [ ] 支持10/30/50/100、自定义、期号、日期和年份查询。
- [ ] 点击任一期可查看与计算核心一致的完整属性。

**Verification：**

- [ ] ViewModel与Compose UI测试通过。
- [ ] 模拟器完成无网首启、联网刷新和历史详情流程。
- [ ] 随机5期界面值与内置JSON一致。
...
- [ ] 图表落点与对应期号位置号码一致。
- [ ] 10/30/50/100及自定义周期驱动同一范围的统计。
- [ ] 颜色不是唯一信息编码，缩放和横向滚动可用。

**Verification：**

- [ ] 图表坐标与统计ViewModel测试通过。
- [ ] 模拟器验证窄屏、100期、大字体和TalkBack标签。
- [ ] 30期/100期统计与领域计算测试结果一致。
...
- [ ] 所有PRD V1筛选条件可组合且结果即时更新。
- [ ] 每个条件显示排除数量，空结果显示冲突和撤销入口。
- [ ] 前导0、组选排列、注数和金额与领域核心一致。

**Verification：**

- [ ] ViewModel测试覆盖增删条件、冲突、倍数边界。
- [ ] Compose UI测试完成典型“胆码+和值+跨度”流程。
- [ ] 1000号码列表滚动无明显卡顿。
...
- [ ] 保存后重启应用可完整还原条件、候选、金额和备注。
- [ ] 回测明确展示覆盖期数、样本不足、平均注数和累计金额，不显示未来概率。
- [ ] 新开奖或官方修正会生成或更新对应复盘。

**Verification：**

- [ ] ViewModel和数据库集成测试通过。
- [ ] 模拟器完成“选号—保存—重启—同步—复盘”主流程。
- [ ] 固定模板重复回测结果完全一致。
...
- [ ] 用户可分别关闭三类提醒，关闭后其他类型不受影响。
- [ ] 同一期同一事件只提醒一次。
- [ ] 所有已定义错误都有可理解文案和可行下一步。

**Verification：**

- [ ] DataStore与提醒去重测试通过。
- [ ] UI测试覆盖无网、解析失败、数据修正和空方案。
- [ ] 日志检查确认不包含用户备注或完整方案。
...
- [ ] PRD主要验收场景全部通过并留存证据。
- [ ] 应用只申请必要网络权限，Release无调试开关和签名材料泄露。
- [ ] 生成可安装APK，记录包名、版本、签名证书摘要和SHA-256。

**Verification：**

- [ ] `.\gradlew.bat clean test lint connectedDebugAndroidTest assembleRelease`
- [ ] 重跑数据全量校验和独立20期抽样。
- [ ] 真机或模拟器完成安装、升级、无网首启和完整主流程。
...
- [ ] 所有自动化测试、lint和Release构建通过。
- [ ] 需求、实现和验收证据一致。
- [ ] APK可安装，数据、方案和主流程真实可用。
- [ ] 产品负责人决定内测或继续修改。
...
产品负责人只需回答：是否批准本计划和对应技术设计进入Task 1。

若批准，下一步是初始化Git和Android工程，不再需要先建设后端或寻找彩报来源。
